function BSProductWeightQty(){
	...
	self.redesignCartRow = function(el, counter, qtyBoxId){
		...
		var pricePerKg = self.roundDecimal(price.substring(1).replace(/,/g, '')/(self.weight));
	...
	self.redesignMiniCartRow = function(el){
		...
		var pricePerUnitCode = self.roundDecimal(price.substring(1).replace(/,/g, '')/(self.weight));
	...
	self.onKeyUpProduct = function(event, actionType, productType, spinnerId, qtyBoxId) {
		if (event.keyCode == 13) {
			self.updateQtyAndPrice(actionType, productType, spinnerId, qtyBoxId);
		}
	};
	self.onKeyUpCart = function(event, actionType, productType, spinnerId, qtyBoxId) {
		if (event.keyCode == 13) {
	...
	self.getWeightQtySpinnerHtml = function(){
		...
		var html = "<div class='qty-spiner'>"+
					...
					"<span><input type='text' value='"+self.weight+"' class='qty-size' id='"+spinnerId+"' onkeyup='customProductWeightQty.onKeyUpProduct(event, 3, "+self.weight+",\""+spinnerId+"\", \""+qtyBoxId+"\")'"+" onblur='customProductWeightQty.updateQtyAndPrice(3,"+self.weight+",\""+spinnerId+"\",\""+qtyBoxId+"\")'/></span>"+
					"<span class='spin'>"+							
	...
	self.getCartWeightQtySpinnerHtml = function(counter){
		...
		var html = "<div class='qty-spiner' style='width:57px;'>"+					
					"<span><input type='text' value='"+value+"' class='qty-size' id='"+spinnerId+"' onkeyup='customProductWeightQty.onKeyUpCart(event, 3, "+self.weight+",\""+spinnerId+"\", \""+qtyBoxId+"\")'"+" onblur='customProductWeightQty.calculateQty(3,"+self.weight+",\""+spinnerId+"\",\""+qtyBoxId+"\")'/></span>"+
					"<span class='spin'>"+							
	...
	self.updatePrice = function(){
		...
		jQuery.ajax({
			...
			success : function(data) {				
				jQuery('.product-shop .price-box span.price').html(data);
			}
		});
	};
...
jQuery( document ).ready(function() {
	...
	jQuery('.cart > form').keypress(function(e){
		if(e.which === 13){
		    return false;
		}
	});
