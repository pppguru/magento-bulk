...
class Idev_OneStepCheckout_Model_Observers_PaypalExpress
{
    ...
    public function skipPaypalPreview($observer) {
        ...
        if(is_object($quote) && is_object($quote->getPayment())){
            //In magento 1.8.1.0, Mage_Paypal_Model_Express_Checkout::PAYMENT_INFO_BUTTON is not defined. Hence we see an undefined error when we use paypal.
			//To fix, we would set the value manually. Mohin - March 15, 2016
			...
			$infoButton = "button";			
			$quote->getPayment()->setAdditionalInformation($infoButton, 0);
        }
    }
