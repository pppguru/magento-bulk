...
class Mage_Sales_Model_Quote extends Mage_Core_Model_Abstract
{
	...
	/**
	 * Get array of all items what can be display directly
	 * Matthew : used "Your cart has been abandoned - Live" email template
	 * @return array
	 */
	public function getFollowUpAllVisibleItems() // Matthew : used AW "Your cart has been abandoned - Live" email template
	{
		$items = array();
		foreach ($this->getItemsCollection() as $item) {
			if (!$item->isDeleted() && !$item->getParentItemId()) {
