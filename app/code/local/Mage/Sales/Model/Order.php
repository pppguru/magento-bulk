...
/*********************** ADDRESSES ***************************/
	...
	public function getFollowUpAllVisibleItems() // Matthew : used AW "Ask for Product Review" email template
	{
		$items = array();
		foreach ($this->getItemsCollection() as $item) {
			if (!$item->isDeleted() && !$item->getParentItemId()) {
				...
				if ($product->getTypeId()=='simple') {
					...
					if (isset($parentIds[0])) {
						...
						$productUrlPath = $parent->getUrlPath();
					} else {
						$productUrlPath = $product->getUrlPath();
					}
				...
				$item->setProducturlpath($productUrlPath);
				$items[] = $item;
			}
		}
		return $items;
...
/*********************** STATUSES ***************************/
	...
	/**
	 * Custom code added by Erik, because of the multiple inheritance issue. app/code/local/bulksupplements/customorder
	 */
