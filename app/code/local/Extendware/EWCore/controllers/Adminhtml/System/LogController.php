...
class Extendware_EWCore_Adminhtml_System_LogController extends Extendware_EWCore_Controller_Adminhtml_Action
{
	public function _construct()
	{
		$this->setMenuPath('ewcore/system/log/log');
		$this->_title($this->__('System'))->_title($this->__('Manage Logs'));
		parent::_construct();
	}
	
	protected function _initLogFile() {
		$model = Mage::registry('ew:current_log_file');
		if (!$model) {
			$model = Mage::getModel('ewcore/system_log_file')->loadById($this->getInput('id'));
	  		Mage::register('ew:current_log_file', $model);
		}
  		return $model;
	}
	...
	public function indexAction() {
	    $this->_initAction();
        $this->_addContent($this->getLayout()->createBlock('ewcore/adminhtml_system_log'));
 	    $this->renderLayout();
	}
	...
	public function editAction() {
		$id     = $this->getInput('id');
		$model  = $this->_initLogFile();
		
		if ($model->getId()) {
			$this->_initAction();
	        	$this->_addContent($this->getLayout()->createBlock('ewcore/adminhtml_system_log_edit'));
	 	    $this->renderLayout();
	 	    return;
		} else {
    ...
    public function deleteAction() {
		...
		if($model->getId()) {
			try {
				...
				$this->_getModuleSession()->addSuccess($this->__('Log file has been deleted'));
				return $this->_redirect('*/*/');
			} catch (Exception $e) {
	...
	public function massDeleteAction() {
		...
		if(is_array($ids) and !empty($ids)) {
			foreach ($ids as $id) {
				try {
					$model = Mage::getModel('ewcore/system_log_file')->loadById($id);
					$model->delete();
					$passCount++;
				} catch (Exception $e) {
					$failCount++;
