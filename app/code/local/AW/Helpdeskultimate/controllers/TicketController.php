...
class AW_Helpdeskultimate_TicketController extends Mage_Adminhtml_Controller_Action
{
        ...
        try {
            if ($data = $this->getRequest()->getPost()) {
                ...
                if (!is_null($uploaders)) {
                    $path = $ticket->getFolderName(true);
        ...
        try {
            ...
            if ($data = $this->getRequest()->getPost()) {
                ...
                if (trim($data['content']) || $this->_getMessagesInPost()) {
                    ...
                    if (!is_null($uploaders)) {
                        $path = $message->getFolderName(true);
                        foreach ($uploaders as $uploader) {
                            $fileName = Mage::helper('helpdeskultimate')->getEncodedFileName($uploader['filename']);
                            $uploader['uploader']->save($path, $fileName);
                        }
        ...
        foreach ($unregistered as $unregUser) {
            ...
            if ($flag) {
                ...
                $options[] = array(
                    ...
                    'is_belong_to_order' => Mage::helper('helpdeskultimate')->isOrderBelongToCustomer($orderId, null, $unregUser->getEmail())
                );
            }
        }
    ...
    {
        $ticket_id = $this->getRequest()->getParam('ticket_id', false);
        $message_id = $this->getRequest()->getParam('message_id', false);

        if ($ticket_id) {
            $ticketModel = Mage::getModel('helpdeskultimate/ticket')->load($ticket_id);
            if ($ticketModel->getId()) {
                $html = $ticketModel->getContent();
                $this->getResponse()->setBody($html);
                return;
            }
        }
        ...
        if ($message_id) {
            $messageModel = Mage::getModel('helpdeskultimate/message')->load($message_id);
            if ($messageModel->getId()) {
                $html = $messageModel->getContent();
                $this->getResponse()->setBody($html);
                return;
            }
        }
