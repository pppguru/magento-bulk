...
class AW_Helpdeskultimate_Helper_Notify extends AW_Helpdeskultimate_Helper_Abstract
{
	...
	public function ticketNew($ticket, $customer = null)
	{
		...
		$ticket->setContent($this->_processContent($ticket->getContent(), array(
			'ticket' => $ticket,
			'department' => $this->_processDepartmentForEmailVariable($department)
		)));
	...
	public function ticketReplyToAdmin($message)
	{
		...
		if ($department->getEnabled() && $department->getNotify()) {
			...
			if (!is_null($mailTemplate)) {
				$this->log(
					'Sending mail about reply in ticket '
					. $ticket->getUid()
					. ' to department '
					. $recipientName . '<' . $recipientEmail . '>'
				);
	...
	public function ticketReassigned($ticket)
	{
		...
		if ($sendReassign && $mailTemplate) {
			...
			$mailTemplate
				...
				->sendTransactional(
					...
					array(
						 'ticket'        => $ticket,
						 'newdepartment' => $this->_processDepartmentForEmailVariable($newDepartment),
		...
		if ($newDepartment->getEnabled() && $newDepartment->getNotify()) {
			...
			if (!is_null($mailTemplate)) {
				$this->log(
					'Sending mail about reassigned ticket '
					. $ticket->getUid()
					. ' to department '
					. $recipientName . '<' . $recipientEmail . '>'
				);
	...
	protected function _processDepartmentForEmailVariable($department)
	{
		$object = new Varien_Object();
		$object->setData($department->getData());
		return $object;
	}
	...
	private function _processAttachments($mail, $folderName, $fileNames)
	{
		if ($fileNames) {
			foreach ($fileNames as $filename) {
				...
				/*$at = $mail->createAttachment(file_get_contents($folderName . $realFilename)); // by default
				$at->disposition = Zend_Mime::DISPOSITION_ATTACHMENT;
				$at->encoding = Zend_Mime::ENCODING_BASE64;
				$at->filename = $filename;*/
				$at = $mail->createAttachment(file_get_contents($folderName . $realFilename), Zend_Mime::TYPE_OCTETSTREAM, null, null, $filename); // Matthew
			}
		}
	}
