...
class AW_Helpdeskultimate_Model_Observer
{
	...
	public function contactFormBotProtect($observer)
	{
		...
		if ($request->getParam('antibot-field', false)) {
			if ($_configHelper->isIntegrationWithContactFormEnabled()) {
				...
				if ($_configHelper->isStandartContactFormDisabled()) {
					$successMessage = Mage::helper('contacts')->__(
		...
		} elseif (Mage::helper('core')->isModuleOutputEnabled('AW_Helpdeskultimate')) {
...
//                $telephoneLine = "\r\n" . Mage::helper('helpdeskultimate')->__('Telephone:') . $post['telephone']; // by default
				$telephoneLine = "\r\n" . Mage::helper('helpdeskultimate')->__('Order Number:') . $post['telephone'];  // Matthew
			...
			/*$ticketSubject = Mage::helper('helpdeskultimate')->__( // by default
				'Contact form %s <%s>',
				trim(isset($post['name']) ? $post['name'] : ''),
				trim(isset($post['email']) ? $post['email'] : '')
			);*/
			$ticketSubject = Mage::helper('helpdeskultimate')->__(trim($post['name'])); // Matthew
	...
	{
		if (Mage::helper('core')->isModuleOutputEnabled('AW_Helpdeskultimate')) {
			$node = Mage::getConfig()->getNode('global/blocks/awcore/rewrite');
			$dnode = Mage::getConfig()->getNode('global/blocks/awcore/drewrite/adminhtml_log_grid');
