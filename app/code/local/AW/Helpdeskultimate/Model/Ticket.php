...
class AW_Helpdeskultimate_Model_Ticket extends AW_Core_Model_Abstract
{
    ...
    public function getAdminFileUrl($file)
    {
        ...
        if (method_exists(Mage::getSingleton('adminhtml/url'), 'getUrl')) {
            return Mage::getSingleton('adminhtml/url')->getUrl('*/*/downloadtck', $params);
        } else {
    ...
    {
        $departmentVariable = new Varien_Object();
        $departmentVariable->setData($this->getDepartment()->getData());
        $tf = Mage::getModel('core/email_template_filter');
        $tf
            ->setUseAbsoluteLinks(true)
            //        ->setUseSessionInUrl(false)
            ->setVariables(array('ticket' => $this, 'department' => $departmentVariable));
        try {
