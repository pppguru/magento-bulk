...
class AW_Helpdeskultimate_Model_Proto extends Mage_Core_Model_Abstract
{
    ...
    public function convertToMessage()
    {
        ...
        if ($ticket = $this->_loadTicket()) {
            ...
            if ($this->getFilename()) {
                @mkdir($message->getFolderName(true));
                foreach (explode('|', $this->getFilename(true)) as $file) {
                    $file = Mage::helper('helpdeskultimate')->getRealFileName($this->getFolderName(true), $file);
                    if ($file) {
                        copy($this->getFolderName(true) . $file, $message->getFolderName(true) . $file);
                    }
                }
    ...
    public function convertToTicket()
    {
        ...
        } elseif (($department = $this->_findDepartment($this->getFromEmail())) || (($this->getSource() == 'order' || $this->getSource() == 'email') && $this->_findTicketIdentity())) {
        ...
        if ($this->getFilename(true)) {
            @mkdir($ticket->getFolderName(true));
            foreach (explode('|', $this->getFilename(true)) as $file) {
                $file = Mage::helper('helpdeskultimate')->getRealFileName($this->getFolderName(true), $file);
                if ($file) {
                    copy($this->getFolderName(true) . $file, $ticket->getFolderName(true) . $file);
                }
            }
     ...
     * @return string
     */
    public function getFolderName($forUpload = false)
    {
        $path = $this->getNewFolderName();

        if (!$forUpload) {
            $realFileName = Mage::helper('helpdeskultimate')->getRealFileName($path, $this->getData('filename'));
            if (!file_exists($path . $realFileName)) {
                $path = $this->getOldFolderName();
            }
        }
    ...
    {
        $path = Mage::getBaseDir('media') . DS . 'helpdeskultimate' . DS;

        $zDate = new Zend_Date($this->getData('created_at'));
        $path .= $zDate->get(Zend_Date::YEAR) . DS;
        $path .= $zDate->get(Zend_Date::MONTH) . DS;
        $path .= $zDate->get(Zend_Date::DAY) .DS;

        $path .= 'proto-' . $this->getId() . DS;
     ...
     * @return string
     */
    public function getOldFolderName()
    {
