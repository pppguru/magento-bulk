...
class AW_Mobile2_Model_Source_Setup extends Mage_Catalog_Model_Resource_Eav_Mysql4_Setup
{
    ...
    protected function _copyMobileProductDescription()
    {
        ...
        if ($oldAttributeId) {
            $newAttributeId = $this->getAttributeId('catalog_product', AW_Mobile2_Helper_Data::PRODUCT_DESCRIPTION_MOBILE_ATTRIBUTE);
    ...
    protected function _copyMobileSettings()
    {
        ...
        $mobile2ConfigPaths = array(
            array(
                AW_Mobile2_Helper_Config::DESIGN_TABLET_LOGO_SRC,
                AW_Mobile2_Helper_Config::DESIGN_MOBILE_LOGO_SRC
            ),
            array(AW_Mobile2_Helper_Config::DESIGN_BOOKMARKS_SRC),
            array(AW_Mobile2_Helper_Config::BEHAVIOR_SWITCHER)
        );
        $adapter = $this->getConnection();
        foreach ($mobile2ConfigPaths as $key => $newConfigPath) {
        ...
        //replace images
        $this->_replaceFile(Mage::getBaseDir('media') . '/awmobile', Mage::getBaseDir('media') . '/aw_mobile2');
        return $this;
    }
