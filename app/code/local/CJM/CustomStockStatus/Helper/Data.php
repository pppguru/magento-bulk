...
class CJM_CustomStockStatus_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	{
		date_default_timezone_set(Mage_Core_Model_Locale::DEFAULT_TIMEZONE);
		$shipbyFormat = Mage::getStoreConfig('custom_stock/configurableproducts/configshipbyformat', Mage::app()->getStore()->getId());
		...
		return $theDate == $today ? Mage::helper('customstockstatus')->__('Today') : strftime($shipbyFormat, $shipDate);
	}

	public function getConfigurableStockStatus($theGoods)
	{
		$storeId = Mage::app()->getStore()->getId();
		...
		if($stocked == 0):
			...
			elseif(Mage::getStoreConfig('custom_stock/configurableproducts/configchildoutofstock', $storeId) && Mage::getStoreConfig('custom_stock/configurableproducts/configchildcustomtext', $storeId) && $stockstatus):
				if($expecdate)
		...
		elseif(Mage::helper('customstockstatus')->getHasDateExpired($isPreorder, 'preorder') !== 'true' && Mage::getStoreConfig('custom_stock/configurableproducts/configpre', $storeId)):
		...
		elseif($qty <= 0):
			...
			if($backordered == 1)
				return $stockstatus && Mage::getStoreConfig('custom_stock/configurableproducts/configchildcustomtext', $storeId) ? $stockstatus : '';
			elseif($expecdate)
		...
		elseif(Mage::getStoreConfig('custom_stock/configurableproducts/configchildcustomtext', $storeId) && $stockstatus):

			return Mage::helper('customstockstatus')->__(' * %s', $stockstatus);

		elseif(Mage::getStoreConfig('custom_stock/configurableproducts/configshipby', $storeId) && Mage::helper('customstockstatus')->getHasDateExpired($isPreorder, 'preorder') === 'true'):

			return Mage::helper('customstockstatus')->__(' * Ships by: %s', Mage::helper('customstockstatus')->getConfigShipDate($shipsin));

		elseif(Mage::getStoreConfig('custom_stock/configurableproducts/configshowstocklevel', $storeId)):
	...
	public function getHasDateExpired($timestamp, $type)
	{
		...
		if($type == 'preorder'):
			...
			return $date > $today ? strftime($dateFormat, $date) : 'true';
		endif;
		if($type == 'expec'):
			$storeId = Mage::app()->getStore()->getId();
	...
	public function getShipCountdownText($shipsin)
	{
		...
		elseif($shipByDateFormated == $todayFormated):
			...
			$shipByDate = strftime($dateFormat, $shipByDate);
		endif;
	...
	public function getShipsByDateMain($businessShipDays, $productkind)
	{
		...
		return date('l F j', $shipByDate) == $today ? Mage::helper('customstockstatus')->__('%s<b>Today</b>', $shipdatetext) : $shipdatetext.'<b>'.strftime($dateFormat, $shipByDate).'</b>';
	}
