<?php
class Bulksupplements_CustomReports_Model_Productsbykg extends Mage_Reports_Model_Mysql4_Product_Sold_Collection
{
...
//			->setOrder('name', self::SORT_ORDER_ASC);
	...
	public function addOrderedQty($from = '', $to = '')
	{
		...
		if (count($childs) == 0 || count($childs[0]) == 0) $childs = '';
		...
		$store_ids = Mage::helper('customreports')->getStoreId();
	...
	//			->where('e.entity_type_id = '.$this->getProductEntityTypeId())
	...
	//			->group('parent_item_id')
			...
			$this->getSelect()->reset()
				...
				->where('e.type_id = ?', 'simple')
				->where('order_items.store_id IN (?)', $store_ids)
				->group('order_items.product_id')
				->having('SUM(order_items.qty_ordered) > ?', 0);
		}

		// Remove capsules products from none capsule parent products,
		// show only for the capsule parent products
		// 22 Nov 2016, Erik,
		// Check mantis: http://mantis.zyloo.com/view.php?id=423
		$parentProductName = Mage::getModel('catalog/product')->load($parentProductId)->getName();
		$isCapsulesProduct = strtolower(substr($parentProductName, -8)) == 'capsules' ? true : false;
		$new_collection = array();
		foreach($this as $product):
			$productName = $product->getName();
			if (!$isCapsulesProduct && strtolower(substr($productName, -8)) == 'capsules'):
				$this->removeItemByKey($product->getId());
			endif;
		endforeach;
