<?php
class Bulksupplements_CustomOrder_Model_Mysql4_Customorder_Order extends Mage_Core_Model_Mysql4_Abstract{
	public function _construct()
	{
		$this->_init('customorder/customorder_order', 'id');
	}
	public function deteleByOrder($order_id,$var){
		$table = $this->getMainTable();
		$where = $this->_getWriteAdapter()->quoteInto('order_id = ? AND ', $order_id).$this->_getWriteAdapter()->quoteInto('`key` = ?  ', $var);
