...
class Ess_M2ePro_Model_Observer_Amazon_Order
{
    ...
    public function revertOrderedQty(Varien_Event_Observer $observer)
    {
        ...
        foreach ($magentoOrder->getAllItems() as $orderItem) {
			...
			$new_quantity = $orderItem->getQtyOrdered() + $stockItem->getQty();
       ...
       WHERE item_stock.product_id = '$product_id' AND item_stock.product_id = '$product_id' " );
       ...
       WHERE item_stock.product_id = '$product_id' AND item_stock.product_id = '$product_id' " ;
			...
			Mage::log($stockItem->getQty(),null,'after_qry.txt');
