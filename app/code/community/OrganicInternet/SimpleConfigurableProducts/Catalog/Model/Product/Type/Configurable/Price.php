...
*/
		/*
		If the request is coming from wishlist, then manually find out the selected child product as it always sends the parent product and this method always retruned 
		the lowest price although the child product of the configurable procut was selected and that led to a wrong price calculation in the cart. From now on, this fix will ensure
		that the currect price is taken. Ref: mantis#338. Mohin - 05 Dec 2015
		*/
		$buyRequest = $product->getCustomOption('info_buyRequest');
		if ($buyRequest) {
			$options = $buyRequest->getItem()->getOptionByCode('simple_product');
			if($options){
				$optionsData = $options->getData();
				if($optionsData && is_array($optionsData)){
					$productId = $optionsData["product_id"];					
		...
		/*Else follow the regular calculation*/
