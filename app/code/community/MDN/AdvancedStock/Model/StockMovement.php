...
class MDN_AdvancedStock_Model_StockMovement extends Mage_Core_Model_Abstract {
    ...
    public function validateStockMovement($productId, $sourceWarehouseId, $targetWarehouseId, $qty) {
        ...
        //remove for compatibility with batch module
        //if ($sourceWarehouseId == $targetWarehouseId)
    ...
    public function createStockMovement($productId, $sourceWarehouseId, $targetWarehouseId, $qty, $description, $additionalDatas = null) {
        try {
            ...
            $obj = mage::getModel('AdvancedStock/StockMovement')
                            ...
                            ->setsm_date(Mage::getModel('core/date')->date('Y-m-d H:i:s'))
            ...
            if (Mage::getSingleton('admin/session')->getUser())
                $obj->setsm_user(Mage::getSingleton('admin/session')->getUser()->getusername());

            $obj->save();

            //log reason if type si stock movement
            if (Mage::getStoreConfig('advancedstock/general/log_adjustment_stock_movement'))
            {
                if ((isset($additionalDatas['sm_type']))
                    &&(($additionalDatas['sm_type'] == 'adjustment')
                    || ($additionalDatas['sm_type'] == 'order'))
                    )
                {
                    $this->logAdjustment($obj);
                }
            }
    ...
    protected function logAdjustment($sm)
    {
        ...
        {
            Mage::getModel('AdvancedStock/StockMovement_Adjustment')
                ->setsm_id($sm->getId())
